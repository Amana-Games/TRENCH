  ...
  const std::vector<std::filesystem::path>& searchPaths)
{
  ...
  {
    // [AMANA GAMES] Search for exposed asset in the editor's own games folder.
    // SystemPaths is in TbUiLib, we are in TbMdlLib. We can use gameConfig.path to get the config location.
    // gameConfig.path typically points to resources/games/AINIMONIA/GameConfig.cfg
    const auto gameDirFile = gameConfig.path.parent_path() / spec.path;
    if (fs::Disk::pathInfo(gameDirFile) == fs::PathInfo::File) {
        return gameDirFile;
    }
...
void Map::loadEntityDefinitions()
{
  if (const auto spec = entityDefinitionFile(*this))
  {
    ...
    const auto path =
      findEntityDefinitionFile(gameConfig, gamePath(), *spec, externalSearchPaths(*this));
